  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'login') {
        ...
        await register(name, username, password, phone, email);
      }
    } catch (err: unknown) {
      ...
      <div className="w-full max-w-sm animate-fade-in-up">
        ...
        >
          <form onSubmit={handleSubmit} className="flex flex-col gap-3">
            ...
            <InputField
              icon="AtSign"
              placeholder={mode === 'login' ? 'Email или имя пользователя' : 'Имя пользователя'}
              value={username}
              onChange={setUsername}
              autoComplete="username"
            />
            {mode === 'register' && (
              <InputField icon="Mail" placeholder="Email (необязательно)" value={email} onChange={setEmail} type="email" autoComplete="email" />
            )}
            <InputField
              icon="Lock"
              placeholder="Пароль"
